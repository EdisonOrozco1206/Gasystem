<x-action-section>
    <x-slot name="title">
        {{ __('Autenticación en 2 factores') }}
    </x-slot>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Acaba de iniciar la autenticación en 2 factores.') }}
                @else
                    {{ __('Has habilitado la autenticación en 2 factores.') }}
                @endif
            @else
                {{ __('No tienes habilitada la autenticación en 2 factores.') }}
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Cuando la autenticación de dos factores esté habilitada, se le solicitará un token aleatorio seguro durante la autenticación. Puede recuperar este token desde la aplicación Google Authenticator de su teléfono.') }}
            </p>
        </div>
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                ...
                @if ($showingConfirmation)
                    <div class="mt-4">
                        <x-label for="code" value="{{ __('C+odigo') }}" />
